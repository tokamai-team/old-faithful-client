...
impl<F: Interceptor + 'static> GrpcClient<F> {
    ...
    pub async fn stream_blocks(
        ...
        if let Some(ref f) = filter_proto {
            info!("Filter: {} accounts to include", f.account_include.len());
            for account in &f.account_include {
                debug!("  - {}", account);
            }
        }
        ...
        info!("Sending StreamBlocksRequest to server...");
        let response = self.client.stream_blocks(request).await?;
        info!("Received response from server, stream established");
        let stream = response.into_inner();

        Ok(Box::pin(Self::map_block_stream(stream)))
    }

    /// Stream blocks in a slot range, returning raw BlockResponse protobuf messages
    pub async fn stream_blocks_raw(
        &mut self,
        start_slot: u64,
        end_slot: Option<u64>,
        filter: Option<StreamBlocksFilter>,
    ) -> GrpcClientResult<
        Pin<Box<dyn Stream<Item = Result<crate::grpc::generated::BlockResponse, Status>> + Send>>,
    > {
        info!(
            "Streaming blocks (raw) from slot {} to {:?}",
            start_slot, end_slot
        );
...
impl GrpcBuilder {
    ...
    fn build(self, channel: Channel) -> GrpcBuilderResult<GrpcClient<InterceptorXToken>> {
        ...
        let mut client = GeneratedClient::with_interceptor(channel, interceptor);

        // Set max decoding message size if specified
        if let Some(size) = self.max_decoding_message_size {
    ...
    pub fn max_decoding_message_size(self, size: usize) -> Self {
        Self {
            max_decoding_message_size: Some(size),
            ..self
        }
    }
}
...
pub async fn connect_with_config(
    ...
    // Set connection timeout (for initial connection only)
    builder = builder.connect_timeout(config.connect_timeout);

    // Set request timeout only if it's reasonable (not for long-running streams).
    // For streaming connections, a very large timeout (like 24 hours) indicates
    // we should rely on keepalive instead of endpoint-level timeout, which can
    // cause DeadlineExceeded errors on long-running streams.
    // For non-streaming RPCs, a reasonable timeout (e.g., 30 seconds) is fine.
    if config.request_timeout < Duration::from_secs(3600) {
        // Only set timeout for non-streaming RPCs (timeout < 1 hour)
        builder = builder.timeout(config.request_timeout);
    }
    // For streaming connections (timeout >= 1 hour), we rely on HTTP2 keepalive
    // to maintain the connection instead of endpoint-level timeout.
    ...
    // Enable keep-alive while idle for streaming connections
    // This prevents the connection from timing out when there's no data flowing
    builder = builder.keep_alive_while_idle(true);

    // Set max decoding message size (important for large Solana blocks)
    builder = builder.max_decoding_message_size(config.max_decoding_message_size);

    // Enable HTTP/2 adaptive window for better performance with large messages
    // This allows the window to grow based on bandwidth-delay product
    builder = builder.http2_adaptive_window(true);

    // Set larger initial window sizes for better throughput with large messages
    // Default is 65535 bytes, we increase to 2MB for streaming large Solana blocks
    builder = builder.initial_connection_window_size(2 * 1024 * 1024);
    builder = builder.initial_stream_window_size(1024 * 1024);
